...
## CRITICAL: Post-Installation Check for Download Failures

**After installation completes, ALWAYS scroll up in your terminal to check for download failures!**

### What to Look For:
The installer will display a prominent section if downloads failed:
```
============================================================
DOWNLOAD FAILURES SUMMARY  
============================================================
The following X items failed to download:
```

**If you see this section, some required models failed to download and need manual intervention.**

### If You See Download Failures:

1. **Don't panic** - ComfyUI will still work, but you'll need to download models manually
2. **Take a screenshot** or save the terminal output showing the failed downloads
3. **Note the specific files** that failed and their target directories
4. **Follow the manual download instructions** provided in the failure summary

### Manual Download Process:

#### Step 1: Identify Failed Downloads
Look for entries like:
```
Failed: Flux Schnell FP8 (REQUIRED)
...
1. **Copy the URL** from the failure summary
2. **Open the URL** in your web browser 
3. **Download the file** (may take 10-30 minutes for large models)
4. **Verify file size** - models should be several GB

#### Step 3: Place Files in Correct Directories
The failure summary will show exactly where each file should go:

- **Flux Models** → Place in `ComfyUI/models/checkpoints/` AND copy to `ComfyUI/user_models/`
- **Text Encoders** → Place in `ComfyUI/models/clip/`
- **VAE Files** → Place in `ComfyUI/models/vae/`
- **Upscalers** → Place in `ComfyUI/models/upscale_models/`

#### Step 4: Restart ComfyUI
After placing files manually, restart ComfyUI to detect the new models.
...
#### Solutions:
1. **Retry the installation** - Sometimes servers are temporarily down
2. **Use a VPN** if certain sites are blocked in your region
3. **Download during off-peak hours** for better speeds
4. **Use a download manager** for large files (recommended for files >1GB)
5. **Check available disk space** - ensure 15GB+ free space
6. **Temporarily disable antivirus** during download (re-enable after)

#### Alternative Download Sources:
If the primary URLs don't work, try these mirrors:
...
2. **Post in Discussions** with your error output
3. **Join the community Discord** for real-time help
4. **Include your system info** and error messages when asking for help
